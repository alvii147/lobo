...
func TestRouterMethods(t *testing.T) {
	...
	getHandler := func(w *flux.ResponseWriter, r *http.Request) {
		getHandlerCallCount++
		w.WriteHeader(http.StatusOK)
	}

	postHandlerCallCount := 0
	postHandler := func(w *flux.ResponseWriter, r *http.Request) {
		postHandlerCallCount++
		w.WriteHeader(http.StatusOK)
	}

	putHandlerCallCount := 0
	putHandler := func(w *flux.ResponseWriter, r *http.Request) {
		putHandlerCallCount++
		w.WriteHeader(http.StatusOK)
	}

	patchHandlerCallCount := 0
	patchHandler := func(w *flux.ResponseWriter, r *http.Request) {
		patchHandlerCallCount++
		w.WriteHeader(http.StatusOK)
	}

	deleteHandlerCallCount := 0
	deleteHandler := func(w *flux.ResponseWriter, r *http.Request) {
		deleteHandlerCallCount++
		w.WriteHeader(http.StatusOK)
	}

	router := flux.NewRouter()
	...
	httpClient := flux.NewHTTPClient(nil)
...
func TestRouterMiddleware(t *testing.T) {
	...
	middleware1 := func(next flux.HandlerFunc) flux.HandlerFunc {
		return flux.HandlerFunc(func(w *flux.ResponseWriter, r *http.Request) {
			callStack = append(callStack, "mw1")
			r.Header.Set("Middleware1", "deadbeef")
			next.ServeHTTP(w, r)
		})
	}
	middleware2 := func(next flux.HandlerFunc) flux.HandlerFunc {
		return flux.HandlerFunc(func(w *flux.ResponseWriter, r *http.Request) {
	...
	handler := func(w *flux.ResponseWriter, r *http.Request) {
...
func TestRouterPathParam(t *testing.T) {
	...
	router.GET(
		"/path/{param}",
		func(w *flux.ResponseWriter, r *http.Request) {
...
func TestRouterRootHeaders(t *testing.T) {
	...
	router := flux.NewRouter(
		flux.WithRouterCORSHeaderNames(wantCORSHeaderNames...),
		flux.WithRouterCORSOrigin(&wantCORSOrigin),
		flux.WithRouterRootHeader("Content-Type", wantContentType),
	)
	router.GET(
		"/path",
		func(w *flux.ResponseWriter, r *http.Request) {
